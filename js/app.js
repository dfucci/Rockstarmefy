var addDetailsToMarkerInfo, cleanTwits, getArtistGigs, getArtistLastTwits, getArtistTwitterUserName, getCurrentArtist, gigs_location, init, m, oauth, player, setMapMaker, showGigsOnMap, sp, update;
...
getArtistTwitterUserName = function(artist) {
  ...
  return jQuery.getJSON(endpoint, {
    ...
    if (data.response.status.code === 0 && (data.response.artist.twitter != null)) {
      ...
      return $("#errors").html("Can't find twitter account");
    }
  });
};
  ...
  return jQuery.getJSON(results.signed_url, function(data) {
    ...
    if (data != null) {
      ...
      for (_i = 0, _len = data.length; _i < _len; _i++) {
        ...
        _results.push($('#tweets ul li').effect('bounce'));
      }
      return _results;
    }
  });
